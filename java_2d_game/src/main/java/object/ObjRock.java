...
public class ObjRock extends Projectile {
    ...
    private static final int OBJ_ROCK_SPRITE_COUNT = 1;
    ...
    public ObjRock(GameWindow gameWindow) {
        super(gameWindow, DIRS.length, OBJ_ROCK_SPRITE_COUNT);
        this.gameWindow = gameWindow;

        setType(new RockType());
        setName("岩");
        setSpeed(1);
        setMaxLife(80);
        setAttack(1);
    ...
    protected void loadSprites() {
        ...
        try {
            ...
            rockSprites = new BufferedImage[DIRS.length][OBJ_ROCK_SPRITE_COUNT];
            fragmentSprites = new BufferedImage[DIRS.length][STONE_SCATTER_SPRITE_COUNT];

            for (int i = 0; i < DIRS.length; i++) {
                for (int j = 0; j < OBJ_ROCK_SPRITE_COUNT; j++) {
                    String path = String.format("object/image-rock%s-%d.gif", DIRS[i].toLowerCase(), j + 1);
    ...
    public BufferedImage getHeldSprite() {
        ...
        if (rockSprites != null && rockSprites.length > downIndex && rockSprites[downIndex].length > frameIndex) {
            BufferedImage src = rockSprites[downIndex][frameIndex];
            // 小さめにリサイズして返す
            int w = ts * 3 / 4;
            int h = ts * 3 / 4;
    ...
    public BufferedImage getThrowSprite(String dir, int frame) {
        ...
        int frameIndex = Math.max(0, Math.min(frame, rockSprites[dirIndex].length - 1));
        return rockSprites[dirIndex][frameIndex];
    }
    ...
    public void draw(Graphics2D g2) {
        ...
        BufferedImage[][] useSprites = shattering ? fragmentSprites : rockSprites;
        ...
        int drawX = getWorldX() - getGameWindow().getPlayer().getWorldX() + getGameWindow().getPlayer().getScreenX();
        int drawY = getWorldY() - getGameWindow().getPlayer().getWorldY() + getGameWindow().getPlayer().getScreenY();
